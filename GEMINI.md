...
作者以多台電腦（跨設備）、多個 AI 代理、多個對話視窗交替開發。

1. ⚠️ **禁止憑對話紀錄/摘要推算版本號與專案狀態**：在更新版本號（`version.json` / `timeline.js` 的 `APP_VERSION`）或修改關鍵指標前，**當下務必先執行 `git log` 或直接檢視實體檔案確認最新狀態**，絕不能單憑聊天紀錄或上下文摘要推算！
2. ⚠️ **禁止自動 `git push`**：修改與測試完成後，**嚴禁自行自動執行 `git push`**。務必先將詳細變更說明與測試成果呈現給作者審閱，**必須等作者審閱並明確指示（例如說「push」或確認無誤）後，才可執行 `git push` 推送到遠端**！
3. ⚠️ **嚴禁過度自動化/擅自修改檔案**：當作者提出疑問、詢問現有實現邏輯、探討時機點或確認狀況時，**僅需做單純的說明與回答，嚴禁擅自進行任何代碼、配置或文件 (.md) 的修改**。只有在作者明確下達變更/修改指令後，方可進行檔案編輯。
