-- ============================================================
-- Add custom_panels column to public.team_plans table
-- and update get_team_plan_by_token RPC
-- Run this in your Supabase Dashboard > SQL Editor
-- ============================================================

-- 1. Add custom_panels column to team_plans if not exists
alter table public.team_plans add column if not exists custom_panels jsonb default '{}';

-- 2. Drop existing function
drop function if exists public.get_team_plan_by_token(text, text);
drop function if exists public.get_team_plan_by_token(text);

-- 3. Create updated RPC returning custom_panels
...
begin
    ...
    if v_plan.share_password is null or v_plan.share_password = '' then
        ...
        select
            ...
            v_plan.custom_mechanics, v_plan.selected_variants, coalesce(v_plan.custom_panels, '{}'::jsonb),
            v_plan.edit_token, v_plan.read_token, v_plan.created_at, v_plan.updated_at,
        ...
        if upper(trim(p_password)) = upper(trim(v_plan.share_password)) then
            ...
            -- Password is required but incorrect or not provided: return metadata only
            return query
            select
